...
cleaned_up_gene_data_folder_path = Path("results/cleaned_gene_data")
if not cleaned_up_gene_data_folder_path.exists():
    cleaned_up_gene_data_folder_path.mkdir(parents=True)

species_gene_data_dict = {species_name: parse_mitochondrial_genome(genbank_filepath=f"genbank_files/{species_name}/{species_name}_mitochondrion.gb") for species_name in all_species_names}
...
TO CREATE THE 'product_to_gene_mapping' DICTIONARY IN THE '0_product_to_gene_mapping.tsv' FILE
...
for species_name in all_species_names:
    ...
    gene_data.to_csv(Path("genbank_files") / f"{species_name}" / f"{species_name}_cleaned_gene_data.tsv", sep="\t", index=False)

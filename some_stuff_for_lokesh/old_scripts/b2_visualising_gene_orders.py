...
combined_gene_orders_df = combined_gene_orders_df[['Arabidopsis_thaliana','Saccharomyces_cerevisiae','Caenorhabditis_elegans','Drosophila_melanogaster',
                                                    'Homo_sapiens','Mus_musculus','Xenopus_tropicalis','Danio_rerio']]
combined_gene_orders_df.to_csv("results/gene_orders/0_combined_gene_orders.tsv", sep="\t", index=False)
...
for species_1, gene_order_list_1 in all_gene_orders.items():
    for species_2, gene_order_list_2 in all_gene_orders.items():
        ...
        plt.xlabel(f"{species_1} genes")
        plt.ylabel(f"{species_2} genes")
        plt.title("Gene Translocation Visualization")
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(folder_path / Path(f"{species_1}_and_{species_2}_possible_gene_translocations"),dpi=300)
        plt.close()
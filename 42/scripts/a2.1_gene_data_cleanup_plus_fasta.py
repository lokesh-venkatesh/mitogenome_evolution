...
for species_name in all_species_names:
    ...
    gene_data = mito.parse_mitochondrial_genome(genbank_filepath=gb_filepath)
    ...
    genome_fasta_string = f">{species_name}:\n{mito.extract_genome_sequence(genbank_filepath=gb_filepath)}"
    mito.write_string_to_file(string=genome_fasta_string, filepath=f"data/genbank_files/{species_name}/{species_name}_mitogenome.fasta")